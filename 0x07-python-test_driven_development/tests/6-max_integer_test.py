...
class TestMaxInteger(unittest.TestCase):
    ...
    def test_float(self):
        ...
        def test_not_list(self):
            """ Test with a param that is not a list: shoukd raise a TypeError"""
            self.assertRaises(TypeError, max_integer, 7)

        def test_unique(self):
            """Test with a list of one int: should return the value of the int"""
            l = [20]
            result = max_integer(l)
            self.assertEqual(result, 20)

        def test_identical(self):
            """Test with a list of identical values int: should return the value"""
            l = [4, 4, 4, 4]
            result = max_integer(l)
